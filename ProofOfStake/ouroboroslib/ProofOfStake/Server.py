...
from FileModule import init_accounts, append_account
...
def add_commitments():
    ...
    required = ['public_key', 'commitment']
    if not all(k in values for k in required):
        return 'Missing values', 400
    address = {
        "address": block_chain.my_profile.address
    }
    block_chain.add_commitment(values['public_key'], values['commitment'])
...
def add_openings():
    ...
    required = ['public_key', 'opening']
    if not all(k in values for k in required):
        return 'Missing values', 400

    block_chain.add_opening(values['public_key'], values['opening'])
