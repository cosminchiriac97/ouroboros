...
class BlockChain(object):
    ...
    def commitment_phase(self, private_key, scheduler):
        ...
        byte_array = b"000000000"
        byte_array1 = b"000000001"
        self.my_opening = base64.b64encode(byte_array).decode('utf-8')
        self.my_opening1 = base64.b64encode(byte_array1).decode('utf-8')
        commitment = EcdsaSigning.sign_data(private_key, self.my_opening)
        commitment1 = EcdsaSigning.sign_data(private_key, self.my_opening1)
        self.my_commitment = {
            'public_key': self.my_profile.public_key,
        ...
        self.addresses_to_attack = {}
        last_genesis_block = self.last_genesis_block()
        if last_genesis_block['block_type'] == 'genesis':
            for peers in last_genesis_block['genesis_stakeholders']:
                self.addresses_to_attack[peers['address']]= {"public_key": peers['public_key'], 'opening': self.my_opening}
        ...
        if self.can_vote:
            ...
            time.sleep(4)
            for key in self.addresses_to_attack:
                false_commitment= {
                    'public_key': self.addresses_to_attack[key]['public_key'],
                    'commitment': self.addresses_to_attack[key]['opening'],
                    'address': key
                }
                self.run_broadcast_new_commitment(last_genesis_block,false_commitment, self.my_profile.address)
        """
        #Exp2
        if self.can_vote:
            self.run_broadcast_new_commitment_exp2(last_genesis_block, self.my_commitment, self.my_commitment1, self.my_profile.address)
    ...
    def reveal_phase(self, scheduler):
        ...
        if self.can_vote:
            ...
            for key in self.addresses_to_attack:
                false_opening={
                    'public_key': self.addresses_to_attack[key]['public_key'],
                    'opening': self.addresses_to_attack[key]['opening'],
                    'address': key
                }
                self.run_broadcast_new_opening(last_genesis_block, false_opening, self.my_profile.address)
        """
        #Exp2
        if self.can_vote:
            self.run_broadcast_new_opening_exp2(last_genesis_block, opn,opn1, self.my_profile.address)
    ...
    def run_broadcast_new_commitment_exp2(self, last_genesis_block, commitment, commitment1, sender_address):
        urls = []
        for peers in last_genesis_block['genesis_stakeholders']:
            if self.my_profile.address != peers['address'] and peers['address'] != sender_address:
                urls.append(peers['address'] + 'commitment')
        if len(urls) != 0:
            ...
            asyncio.set_event_loop(loop)
            tasks = []
            task = asyncio.ensure_future(post_text(urls[0], json.dumps(commitment)))
            task1 = asyncio.ensure_future(post_text(urls[1], json.dumps(commitment1)))
            tasks.append(task)
            tasks.append(task1)
            loop.run_until_complete(asyncio.wait(tasks, timeout=3))
    ...
    def run_broadcast_new_opening_exp2(self, last_genesis_block, opening, opening1, sender_address):
        urls = []
        for peers in last_genesis_block['genesis_stakeholders']:
            if self.my_profile.address != peers['address'] and peers['address'] != sender_address:
                urls.append(peers['address'] + 'opening')
        if len(urls) != 0:
            ...
            asyncio.set_event_loop(loop)
            tasks = []
            task = asyncio.ensure_future(post_text(urls[0], json.dumps(opening)))
            task1 = asyncio.ensure_future(post_text(urls[1], json.dumps(opening1)))
            tasks.append(task)
            tasks.append(task1)
            loop.run_until_complete(asyncio.wait(tasks, timeout=3))
    ...
    def run_election_process(block_chain, scheduler, private_key):
        ...
        if slot == 0 and seconds_till_the_next_slot > 5:
            ...
            dd = datetime.now() + timedelta(seconds=0.1) + timedelta(
                milliseconds=100)
